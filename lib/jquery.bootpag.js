...
(function($, window) {
    ...
    $.fn.bootpag = function(options){
        ...
        settings.maxVisible = parseInt(settings.maxVisible);
        settings.maxVisible = (settings.maxVisible % 2) ? settings.maxVisible : settings.maxVisible - 1;
        ...
        return this.each(function(){
            ...
            var $bootpag, lp, me = $(this),
                ...
                offset = (settings.maxVisible - 1) / 2,
                start = 1,
                end = settings.maxVisible;

            if (settings.page > (settings.maxVisible - offset)) {
                start = settings.page - offset;
                end = offset + settings.page;
            }
            
            if (settings.page >= (settings.total - offset))
                start = (settings.total - settings.maxVisible) + 1;
            
            end = Math.min(end, settings.total);
